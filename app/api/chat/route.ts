...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    // 6. 非流式处理
    const response = await agentService.processMessage(finalSessionId, message);
    ...
    const chatResponse: ChatResponse = {
      ...
      data: {
        ...
        metadata: {
          sessionId: finalSessionId,
          model: response.data?.metadata.model || 'claude-3-5-sonnet-20241022',
          tokens: {
            input: response.data?.metadata.tokensUsed || 0,
            output: response.data?.metadata.tokensUsed || 0,
          },
          processingTime,
        },
      },
  ...
  const stream = new ReadableStream({
    async start(controller) {
      try {
        ...
        // 处理消息流
        const messageStream = agentService.processMessageStream(sessionId, message);
  ...
  try {
    ...
    const overallStatus = 
      configHealth.status === 'error' || serviceHealth.status === 'unhealthy' 
        ? 'unhealthy' 
        : configHealth.status === 'warning' || serviceHealth.status === 'degraded'
        ? 'degraded'
        : 'healthy';
